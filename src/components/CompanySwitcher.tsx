    ...
    return (
        <div className="relative px-3 mb-4">
            <button
                ...
                className="w-full flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-darkbg-light border border-slate-200 dark:border-slate-800 hover:border-primary/50 transition-all group"
            >
                <div className="flex items-center space-x-3 overflow-hidden">
                    ...
                    <div className="flex flex-col items-start overflow-hidden">
                        <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Company</span>
                        <span className="text-sm font-bold text-slate-800 dark:text-white truncate">
                            {currentCompany?.name || 'Select Company'}
                        </span>
                    </div>
                </div>
                <ChevronUpDownIcon className="h-5 w-5 text-slate-400 group-hover:text-primary transition-colors" />
            </button>

            {isOpen && (
                <div className="absolute left-3 right-3 top-full mt-2 z-50 bg-white dark:bg-darkbg-surface border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="p-2 space-y-1 max-h-60 overflow-y-auto custom-scrollbar">
                        {companies.map((company) => (
                            ...
                            >
                                ...
                                {company.id === user?.current_company_id && <CheckIcon className="h-4 w-4" />}
                            </button>
                        ))}
