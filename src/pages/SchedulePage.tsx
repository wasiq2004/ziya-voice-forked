...
const SchedulePage: React.FC = () => {
    ...
    // Form inputs
    const [assignForm, setAssignForm] = useState({ leadName: '', phone: '', agentId: '', time: '' });
    ...
    const handleAssignCall = () => {
        ...
        setAssignForm({ leadName: '', phone: '', agentId: '', time: '' });
    };
        ...
        >
            ...
            {activeTab === 'schedule' && (
                <div className="space-y-6">
                    {/* Filters */}
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
                        <div className="relative w-full md:w-96">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50"
                            />
                        </div>
                        <div className="flex items-center space-x-3 w-full md:w-auto overflow-x-auto">
                            <FunnelIcon className="h-5 w-5 text-slate-400 flex-shrink-0" />
                            {['All', 'Scheduled', 'Completed', 'Missed', 'Rescheduled'].map(status => (
                                <button
                                    key={status}
                                    onClick={() => setFilterStatus(status)}
                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-colors ${filterStatus === status ? 'bg-primary/10 text-primary' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                                >
                                    {status}
                                </button>
                            ))}
