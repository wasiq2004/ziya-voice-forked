...
const LoginPage: React.FC = () => {
  ...
  const [resetStep, setResetStep] = useState<'request' | 'verify' | 'update'>('request');
  ...
  const openResetFlow = (initialEmail?: string, step: 'request' | 'verify' | 'update' = 'request', message = '') => {
    const nextEmail = (initialEmail || email || '').trim().toLowerCase();
    setResetMode(true);
    setResetStep(step);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        setSuccessMessage('Identity created! Securing link...');
        ...
        if (error) throw error;
        setSuccessMessage('Login successful! Securing connection...');
        // AuthContext already handles navigation on signIn, so we just wait for it.
        return;
      }
    } catch (err: any) {
      setError(err.message || 'Failed to authenticate');
      if (!isSignUp && err.passwordResetInitiated) {
        openResetFlow(err.email || email, 'verify', err.message || 'OTP sent to your email. Verify it to reset your password.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleRequestPasswordReset = async () => {
    ...
    try {
      ...
      setResetMode(true);
      setResetStep('verify');
      setResetEmail(result.email || targetEmail);
      setResetMessage(result.message || 'OTP sent. Please check your email.');
    } catch (err: any) {
  ...
  const handleVerifyPasswordResetOtp = async () => {
    ...
    try {
      const result = await authService.verifyPasswordResetOtp(targetEmail, targetOtp);
      setResetStep('update');
      setResetMessage(result.message || 'OTP verified. Enter your new password.');
    } catch (err: any) {
  ...
  const handleConfirmPasswordReset = async () => {
    ...
    if (!targetEmail || !targetOtp || !newPassword || !confirmNewPassword) {
    ...
    try {
      const result = await authService.confirmPasswordReset(targetEmail, targetOtp, newPassword);
      setResetMode(false);
      setPassword('');
      setResetOtp('');
      setNewPassword('');
      setConfirmNewPassword('');
      setSuccessMessage(result.message || 'Password updated successfully. Please log in.');
    } catch (err: any) {
        ...
        <div key={isSignUp ? 'signup' : 'login'} className="w-full max-w-md animate-scale-in" style={{ animationDelay: '0.1s' }}>
          ...
          <div className="glass-panel-dark rounded-[2rem] shadow-2xl p-8 sm:p-10 border border-white/10 relative overflow-hidden">
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <div className="animate-slide-left group" style={{ animationDelay: isSignUp ? '0.5s' : '0.4s' }}>
                <div className="flex justify-between items-center mb-2 ml-1">
                  ...
                  {!isSignUp && (
                    <button
                      type="button"
                      onClick={() => openResetFlow(email, 'request', 'Request an OTP to reset the password for this email address.')}
              ...
              {!isSignUp && resetMode && (
                <div className="animate-slide-up rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-5 space-y-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h3 className="text-sm font-bold text-cyan-100">Password Reset</h3>
                      <p className="mt-1 text-xs text-cyan-100/80">
                        {resetStep === 'request' && 'Send an OTP to the account email address.'}
                        {resetStep === 'verify' && 'Enter the OTP sent to your email to continue.'}
                        {resetStep === 'update' && 'OTP verified. Set your new password below.'}
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="space-y-4">
                    <div className="group">
                      <label className="block text-sm font-semibold text-slate-300 mb-2 ml-1">Account Email</label>
                      <input
                        type="email"
                        value={resetEmail}
                        onChange={(e) => setResetEmail(e.target.value)}
                        className="w-full px-4 py-3.5 bg-slate-900/50 border border-slate-700/50 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary text-white transition-all outline-none placeholder-slate-500 shadow-inner"
                        placeholder="Enter your email"
                      />
                    </div>
                    ...
                    {resetStep !== 'request' && (
                      <div className="group">
                        <label className="block text-sm font-semibold text-slate-300 mb-2 ml-1">OTP</label>
                        <input
                          type="text"
                          inputMode="numeric"
                          maxLength={6}
                          ...
                          className="w-full px-4 py-3.5 bg-slate-900/50 border border-slate-700/50 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary text-white transition-all outline-none placeholder-slate-500 shadow-inner tracking-[0.35em]"
                        ...
                        <div className="group">
                          ...
                          <div className="relative">
                            <input
                              ...
                              className="w-full px-4 py-3.5 pr-12 bg-slate-900/50 border border-slate-700/50 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary text-white transition-all outline-none placeholder-slate-500 shadow-inner"
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row">
                    {resetStep === 'request' && (
                      <button
                        ...
                        className="w-full py-3 rounded-xl text-white font-semibold bg-cyan-500/90 hover:bg-cyan-500 transition-colors disabled:opacity-60"
                      >
                    ...
                    {resetStep === 'verify' && (
                      <>
                        ...
                        <button
                          ...
                          onClick={handleRequestPasswordReset}
                          disabled={resetLoading}
                          className="w-full py-3 rounded-xl text-slate-200 font-semibold border border-white/10 bg-white/5 hover:bg-white/10 transition-colors disabled:opacity-60"
                        >
                          Resend OTP
                        </button>
                      </>
